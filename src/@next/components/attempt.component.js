 ...
 */
export default function Attempt({ valid, hidden, skeleton, index, isLast, text }) {
  ...
  if (skeleton) {
    ...
    if (isLast) {
      return <Layout style={styles.layout}>
        ...
        <Layout style={{ ...SkeletonStyles.textSkeleton, height: 3, width: skWidth }} />
      </Layout>
    } else {
      return <Layout style={styles.layout}>
        <Icon style={styles.icon} name='close-outline' />
        <Layout style={{ ...SkeletonStyles.textSkeleton, height: 2, width: skWidth }} />
      </Layout>
    }
  }
  if (hidden === true) {
